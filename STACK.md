...
**Perception: what counts as a sighting.** Three answers, one interface (`SearchTask.step(seen_px=...)`). The *geometric* sensor is the default and the one every policy in the library was trained against: a cone, an effective range scaled by the target's contrast and the foliage in the way, a line-of-sight trace over terrain and buildings, and a dropout probability -- the truth, softened. The *pixel* sensor asks the renderer's instance segmentation how much of a hull is in frame. The *detector* (`vesper.lab.detector`, `scripts/detect_server.py`) hands the rendered RGB to the trained RF-DETR and takes its boxes: the policy then knows a vehicle exists only when the network calls it, which is the same sensor that will fly. The detector cannot live in the Isaac container -- rfdetr installs into the venv under `scratch/`, and the container forgets pip installs -- so it runs as its own process on the droplet and the sim posts frames over the loopback (host networking; no frame leaves the box). Boxes carry no identity, so each vehicle's true position is projected into the tile and claims the smallest box that lands on it: the truth answers *which* target a detection belongs to, never *whether* there was one. Measured on the RTX 6000 Ada, RF-DETR Nano serves 64 tiles at 384 px in ~100 ms (1.6 ms/image), so a detector-in-the-loop run is dozens of environments rather than a thousand, and `det_recall` (what share of the targets the renderer put in frame the network actually found) rides in the training curve beside the reward.
